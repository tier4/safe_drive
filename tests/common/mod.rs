...
pub type Request = add_three_ints::sample_msg__srv__AddThreeInts_Request;
pub type Response = add_three_ints::sample_msg__srv__AddThreeInts_Response;
...
pub fn create_server(node: Arc<Node>, service_name: &str) -> Result<ServerType, Box<dyn Error>> {
    let server = node.create_server(
        service_name,
        unsafe { add_three_ints::rosidl_typesupport_c__get_service_type_support_handle__sample_msg__srv__AddThreeInts() as *const()},
        None,
    )?;
